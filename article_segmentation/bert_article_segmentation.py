...
if numpy_version.startswith('2.'):
    print("\nWARNING: NumPy 2.x detected. This version is incompatible with transformers library.")
    print("Options:")
    print("1. Continue with fallback mode (no sentence transformer)")
...
# Workaround for "numpy.dtype size changed" error
...
def clean_line(line):
    ...
    #  BERT tokenizer handle lowercasing or stripping punctuation 
    return line
...
for line in ocr_lines:
    s_embed = get_embeddings(line) # s_embed, h_embed = get_embeddings(line)
    sent_embeddings.append(s_embed)
    # hist_embeddings.append(h_embed)

print(f"Generated embeddings for {len(sent_embeddings)} lines")

print("Sentence embedding dimension:", sent_embeddings[0].shape)
...
sent_tensor = torch.tensor(np.array(sent_embeddings), dtype=torch.float32)
# hist_tensor = torch.tensor(np.array(hist_embeddings), dtype=torch.float32)

# Get original dimensions
D_sent = sent_tensor.shape[1]
...
with torch.no_grad():
    proj_sent_tensor = proj_sent(sent_tensor)
    # proj_hist_tensor = proj_hist(hist_tensor)
# print("Applying concatenation for fusion...") # No longer concatenating
print("Using projected BERT embeddings.")
with torch.no_grad():
    # fused_outputs will have shape (num_lines, common_dim)
    fused_outputs = proj_sent_tensor # torch.cat((proj_sent_tensor, proj_hist_tensor), dim=1)

print(f"Processed BERT embeddings generated with shape: {fused_outputs.shape}")
...
processed_bert_np = fused_outputs.detach().cpu().numpy()

# fused_subset = fused_np[:200] # Renamed fused_np to processed_bert_np
processed_bert_subset = processed_bert_np[:200]


# Reduce dimensions
pca = PCA(n_components=2)
# fused_2d = pca.fit_transform(fused_subset) # Renamed variables
processed_bert_2d = pca.fit_transform(processed_bert_subset)

# Plot
plt.figure(figsize=(10, 7))
# plt.scatter(fused_2d[:, 0], fused_2d[:, 1], alpha=0.7) # Renamed variables
plt.scatter(processed_bert_2d[:, 0], processed_bert_2d[:, 1], alpha=0.7)
# plt.title("Fused Embedding Space (BERT + Historical, Concatenated)") # Updated title
plt.title("Projected BERT Embedding Space")
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.grid(True)
# plt.savefig("fused_emebddings.png") # Updated filename
plt.savefig("projected_bert_embeddings.png")
...
with torch.no_grad():
    ...
    # contextualized_outputs = non_parametric_self_attention(fused_outputs.unsqueeze(0))[0] # This was already correct
    contextualized_outputs = non_parametric_self_attention(fused_outputs.unsqueeze(0))[0]

print(f"Contextualized BERT embeddings generated with shape: {contextualized_outputs.shape}")
...
# No ground truth boundary optimization - use simple threshold
...
# Default to the original method, but allow selection
# In a real interactive session, we'd prompt for input here
# For now, we'll use the smoothed local minima method as it combines both improvements
selected_method = 1  # Change this to select a different method
...
if len(segment_embeddings) > 1:
    ...
    cluster_labels = clusterer.fit_predict(segment_embeddings)

    print(f"HDBSCAN detected {len(set(cluster_labels)) - (1 if -1 in cluster_labels else 0)} clusters")
...
if gt_articles and segments:
    ...
    if np.sum(mask) > 0:
        ...
        # Save B-Cubed metrics to a CSV file for aggregation across all files
        import os
        csv_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../b_cubed_scores-attention2stepbert-hdbmode1.csv')
