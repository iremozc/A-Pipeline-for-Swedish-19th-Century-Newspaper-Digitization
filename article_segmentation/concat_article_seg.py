...
for line in ocr_lines:
    s_embed, h_embed = get_embeddings(line)
...
with torch.no_grad():
    ...
    fused_outputs = torch.cat((proj_sent_tensor, proj_hist_tensor), dim=1)
...
# The original FusionLayer (MLP) is commented out.
# The NonParamCrossAttention requires two inputs for cross-attention,
# but we have already concatenated the embeddings into fused_tensor.
# We will proceed directly to self-attention on the concatenated tensor.
...
plt.title("Fused Embedding Space (BERT + Historical, Concatenated)")
...
# --- Smoothing logic from attention2stephistfixedfasttext-smooth.py ---
...
thresholds = {
    ...
    "mean-2.0*std": np.mean(similarities) - 2.0 * np.std(similarities),
    "fixed-0.5": 0.5
}
...
# --- Smoothing and advanced boundary detection logic (from fasttext-smooth) ---
...
if gt_articles and segments:
    ...
    if np.sum(mask) > 0:
        ...
        import os
        csv_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../b_cubed_scores-attention2stepspetralmode1.csv')
